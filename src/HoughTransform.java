...
public class HoughTransform {
	...
	public void houghTransformCircleNoR(BufferedImage image) throws IOException {
		...
		for (int r = 10; r < 60; r=r+2) {
			for (int i = 0; i < listI.size(); i++) {

				for (int angle = 0; angle < theta; angle = angle + 10) {
					double t = angle * Math.PI / 180;
					int x = (int) (listI.get(i) + r * Math.cos(t));
					int y = (int) (listJ.get(i) + r * Math.sin(t));
		...
		List<HoughCircle> peakCircle = peakHoughCircle(hist, 80);
		...
		for (int k = 0; k < peakCircle.size(); k++) {
			...
			for (int i = 0; i < listI.size(); i++) {
				int tmpI = listI.get(i);
				int tmpJ = listJ.get(i);
				int tmpR = (int) Math.sqrt((tmpI - x) * (tmpI - x) + (tmpJ - y) * (tmpJ - y));
				if (tmpR == r && data[tmpI][tmpJ] != 0) {
					data[tmpI][tmpJ] = setRed();
				} else {
					data[tmpI][tmpJ] = setColor(data[tmpI][tmpJ]);
				}

			}
		}
		this.mat2Image2("./image/houghCircle.png", data);
	}
	...
	public List<HoughCircle> peakHoughCircle(int[][][] mat, int num) {
		...
		for (int r = 0; r < ro; r++) {
			for (int i = 0; i < width; i++) {
				for (int j = 0; j < height; j++) {
					if (mat[r][i][j] > (int) (max * 1.0 * num / 100.0))
	...
	public void mat2Image2(String fileName, int[][] mat) throws IOException {
		int width = mat[0].length;
		int height = mat.length;
		BufferedImage bi = new BufferedImage(height, width, BufferedImage.TYPE_INT_RGB);
		for (int i = 0; i < width; i++) {
			for (int j = 0; j < height; j++) {
				bi.setRGB(j, i, mat[j][i]);
			}
		}
